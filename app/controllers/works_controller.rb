class WorksController < ApplicationController
  ...
  def index
    ...
    @works = Work.where('user_id::text LIKE?', "#{current_user.id}").order(updated_at: "DESC")
  end
  ...
  def search
    ...
    if params[:keyword].present?
      @works = Work.where(['user_id::text like? AND name like?', "#{current_user.id}", "%#{params[:keyword]}%"]).order(updated_at: "DESC")
